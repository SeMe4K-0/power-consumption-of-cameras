...
func (r *Repository) GetDraftRequestCamerasCalculationInfo(userID uint) (ds.RequestCamerasCalculation, []ds.CamerasCalculation, error) {
	requestCamerasCalculation := ds.RequestCamerasCalculation{}
	err := r.db.Preload("Creator").Preload("Moderator").Where("creator_id = ? AND status = ?", userID, ds.RequestStatusDraft).First(&requestCamerasCalculation).Error
	if err != nil {
		return ds.RequestCamerasCalculation{}, nil, err
	}
...
func (r *Repository) FormDraft(id uint, userID uint) error {
	draft, calcs, err := r.GetDraftRequestCamerasCalculationInfo(userID)
...
func (r *Repository) CreateRequestCamerasCalculationWithCamera(cameraID uint, power float64, userID uint) (ds.RequestCamerasCalculation, error) {
	requestCamerasCalculation := ds.RequestCamerasCalculation{
