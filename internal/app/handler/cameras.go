...
func (h *Handler) AddCameraToRequestCamerasCalculationAPI(ctx *gin.Context) {
	...
	draft, _, err := h.Repository.GetDraftRequestCamerasCalculationInfo()
	if err != nil {
		created, createErr := h.Repository.CreateRequestCamerasCalculationWithCamera(uint(id), req.Power)
		if createErr != nil {
